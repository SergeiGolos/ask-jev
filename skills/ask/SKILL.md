...
- `{{file}}`/`{{filename}}`/`{{content}}` come from `-f`; `-t name=value` sets any other token, overriding front-matter `args`. A referenced token with no value hard-errors in non-TTY: pass every `-t`. Tokens are Mustache `{{name}}` syntax.
